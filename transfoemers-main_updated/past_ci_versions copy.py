...
#    os.system(f'echo "export INSTALL_CMD=\'{info["install"]}\'" >> ~/.profile')
    os.environ["INSTALL_CMD"] = info["install"]
    with open("~/.profile", "a") as f:
        f.write("export INSTALL_CMD='{}'\n".format(info["install"]))
        f.close()

    print("export INSTALL_CMD='{}' >> ~/.profile".format(info["install"]))

    cmd = ["echo", "export INSTALL_CMD='{}'".format(info["install"]), ">>", "~/.profile"]
    subprocess.call(cmd)
...
#    os.system(f"echo \"export CUDA='{cuda}'\" >> ~/.profile")
    if "cuda" in info:
        os.environ["CUDA"] = info["cuda"]
        with open("~/.profile", "a") as f:
            f.write("export CUDA='{}'\n".format(info["cuda"]))
            f.close()

        print("export CUDA='{}' >> ~/.profile".format(info["cuda"]))

        cmd = ["echo", "export CUDA='{}'".format(info["cuda"]), ">>", "~/.profile"]
        subprocess.call(cmd)
