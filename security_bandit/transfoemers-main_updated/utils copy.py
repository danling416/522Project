                ...
                with ZipFile(output_path, "r") as zip_file:
                    for member in zip_file.infolist():
                        if member.filename.startswith('/') or '..' in member.filename:
                            raise ValueError(f"Unsafe filename: {member.filename}")
                        zip_file.extract(member, output_path_extracted)
                ...
                with tarfile.open(output_path, "r") as tar_file:
                    for member in tar_file.getmembers():
                        ...
                        if member.issym() or member.islnk():
                            raise ValueError(f"Unsafe symlink or hard link: {member.name}")
                        if member.name.startswith('/') or '..' in member.name:
                            raise ValueError(f"Unsafe filename: {member.name}")
                        tar_file.extract(member, output_path_extracted)
            else:
