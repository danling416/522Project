...
#    os.remove(archive_path)
    ...
    with tarfile.open(archive_path, "r:gz") as tar:
        for member in tar.getmembers():
            # Check if member file paths are safe to extract
            if ".." in member.name:
                raise ValueError("Unsafe member file path: {}".format(member.name))
            # Extract member files only to the intended directory
